...
public class Board {
	...
	private void initEmptyPieces()
	{
		...
		{
			for (int x = 0; x < 8; x++){
				pieces[x][y] = new EmptyPiece(this, Color.BLUE, new Coordinate(x, y));
			}
		}
	}
	...
	{
		pieces[location.getX()][location.getY()] = new EmptyPiece(this, Color.BLUE, location);
	}
	...
	public boolean movePiece(Coordinate location1, Coordinate location2)
	{
		...
		if ((piece2.getName().equals("") || !(piece1.getColor().equals(piece2.getColor()))) && (piece1.moveable(location2)))
		{
	...
	public boolean isInCheckmate(Color playerColor){
		...
		for (x = 0; x < 8; x++)
		{
			opponentLocation = new Coordinate(friendLocation.getX() + moves[2*x], friendLocation.getY() + moves[2*x+1]);
			if (!((0 <= opponentLocation.getX()) && (opponentLocation.getX() <= 7) 
					&& (0 <= opponentLocation.getY()) && (opponentLocation.getY() <= 7)))
			{
				// Move out of bounds
				continue;
			}
		...
		for (x = 0; x < friendlyPieces.size(); x++)
		{
			...
			for (y = 0; y < opponentPieces.size(); y++)
			{
				...
				{
					tmpBoard.pieces = this.pieces.clone();
					tmpPiece = tmpBoard.getPiece(friendLocation); // Get tmp Friendly Piece
					tmpBoard.setPiece(tmpPiece, opponentLocation);
					tmpBoard.removePiece(friendLocation);
